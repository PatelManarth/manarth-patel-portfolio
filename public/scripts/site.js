...
function activateRoute(routeName) {
  ...
  if (!route) return;
  $$('[data-route]').forEach(item => item.classList.toggle('is-active', item.dataset.route === routeName));
  const output = $('[data-route-output]');
  if (output) output.innerHTML = `<strong>${route.title}</strong><p>${route.text}</p><a class="button button--primary" href="${route.href}">${route.label}</a>`;
  localStorage.setItem('portfolio-route', routeName);
}
...
if (savedRoute) activateRoute(savedRoute);

const typewriter = $('[data-typewriter]');
if (typewriter && !matchMedia('(prefers-reduced-motion: reduce)').matches) {
  const commands = ['reviewing project evidence...', 'mapping identity and endpoint controls...', 'loading analyst decision paths...', 'indexing labs, notes and articles...'];
  let commandIndex = 0;
  ...
  const type = () => {
    ...
    if (!deleting && characterIndex === command.length) { deleting = true; setTimeout(type, 1300); return; }
    if (deleting && characterIndex === 0) { deleting = false; commandIndex = (commandIndex + 1) % commands.length; }
...
if (greeting) {
  const messages = ['Evidence is strongest when the reasoning is visible.', 'Explore the portfolio in simple or technical depth.', 'Synthetic scenarios are clearly labelled; project evidence stays truthful.', 'Search any tool, control, project, framework or concept.'];
  greeting.textContent = messages[Math.floor(Math.random() * messages.length)];
}
...
$$('[data-nist]').forEach(button => button.addEventListener('click', () => {
  $$('[data-nist]').forEach(item => item.classList.toggle('is-active', item === button));
  const detail = $('[data-nist-detail]');
  const item = nistContent[button.dataset.nist];
  if (detail && item) detail.innerHTML = `<span class="eyebrow">Framework explorer</span><h3>${item.title}</h3><p>${item.text}</p><ul>${item.items.map(value => `<li>${value}</li>`).join('')}</ul>`;
}));

const triageFeedback = {
  disable:'Premature. Immediate disabling may be justified later, but first confirm the account, source, MFA result, timing and related activity unless the risk is already critical.',
  context:'Best first step. Establish context before assigning confidence or containment: source IP, user behavior, location, MFA, device, time and nearby events.',
  ignore:'Unsafe. A successful sign-in after failures can still indicate password spraying, credential compromise or a legitimate user mistake that needs validation.',
  restart:'Not relevant as the first action. The evidence is authentication-focused; restarting an endpoint does not explain the sign-in pattern.'
};
$$('[data-triage-choice]').forEach(button => button.addEventListener('click', () => {
  $$('[data-triage-choice]').forEach(item => item.classList.toggle('is-selected', item === button));
  const feedback = $('[data-triage-feedback]');
  if (feedback) feedback.textContent = triageFeedback[button.dataset.triageChoice];
}));
...
if (canvas && !matchMedia('(prefers-reduced-motion: reduce)').matches) {
  const context = canvas.getContext('2d'); let width; let height; let nodes=[];
  function resize(){width=innerWidth;height=innerHeight;canvas.width=width;canvas.height=height;nodes=Array.from({length:Math.min(65,Math.max(28,Math.floor(width*height/26000)))},()=>({x:Math.random()*width,y:Math.random()*height,vx:(Math.random()-.5)*.18,vy:(Math.random()-.5)*.18}));}
  addEventListener('resize',resize);resize();
  (function draw(){context.clearRect(0,0,width,height);const rgb=html.dataset.theme==='green'?'27,233,140':'45,124,255';nodes.forEach((node,index)=>{node.x+=node.vx;node.y+=node.vy;if(node.x<0||node.x>width)node.vx*=-1;if(node.y<0||node.y>height)node.vy*=-1;context.fillStyle=`rgba(${rgb},.65)`;context.beginPath();context.arc(node.x,node.y,1.4,0,Math.PI*2);context.fill();for(let next=index+1;next<nodes.length;next+=1){const other=nodes[next];const distance=Math.hypot(node.x-other.x,node.y-other.y);if(distance<125){context.strokeStyle=`rgba(${rgb},${(1-distance/125)*.14})`;context.beginPath();context.moveTo(node.x,node.y);context.lineTo(other.x,other.y);context.stroke();}}});requestAnimationFrame(draw);}());
}